...
export function useTable<Filters extends Record<string, any>>(
  ...
  const filters = ref<Filters>(deepClone(getDefaultFilters(defaultFilters))) as Ref<Filters>
  const formState = ref<Filters>(deepClone(getDefaultFilters(defaultFilters))) as Ref<Filters>
